...
export default function Leaderboard() {
	...
	return (
		<div className="container mx-auto px-4 py-8 w-full">
			...
			<h1 className="text-3xl mb-4 font-bold text-center">Printing Legion Leaderboard</h1>
			<p className="text-lg text-center text-gray-700 mb-10">
				Top Hack Club 3D printers ranked by total grams printed. Stats refresh periodically
				straight from the legion API.
			</p>

			{loading && (
				<div className="text-center text-2xl py-10">fetching legends...</div>
			)}
				...
				<>
					<section className="grid gap-6 md:grid-cols-3 mb-12">
						{podium.map((entry) => (
							<article
								key={entry.slack_id}
								className="bg-white rounded-3xl shadow-lg p-6 flex flex-col items-center text-center border border-blue-50"
							>
								...
								<p className="text-gray-500 mb-4">{formatWeight(entry.total_grams)}</p>
								<a
					...
					{rest.length > 0 && (
						<section className="bg-white rounded-2xl shadow divide-y divide-gray-100 overflow-hidden">
