...
export default function PrinterCard({ printer }) {
  let { slack_id, nickname, profile_pic, website, bio, country, total_prints, total_grams } =
    printer;
      ...
      <div className="p-4 flex items-start gap-4">
          ...
          <div className="w-full flex flex-col gap-2">
            <div className="rounded-md bg-light-bg-2 px-2 py-1 text-sm text-light-tx">
              <p className="leading-tight">
                <span className="font-semibold">{total_prints ?? 0}</span> prints
              </p>
            </div>
            <div className="rounded-md bg-light-bg-2 px-2 py-1 text-sm text-light-tx">
              <p className="leading-tight">
                <span className="font-semibold">{formatWeight(total_grams)}</span>
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="min-w-0 flex-1 flex flex-col gap-2">
          <div>
            ...
            {country && <p className="text-xs text-light-tx-2 leading-tight">{country}</p>}
          </div>

          {bio && <p className="text-sm text-light-tx leading-relaxed">{bio}</p>}
